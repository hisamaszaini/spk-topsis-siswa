...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6 bg-white border-b border-gray-200">
        <h1 class="text-2xl font-bold mb-6">Tambah Sub Kriteria</h1>
        ...
        <form action="{{ route('sub-kriteria.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="id_kriteria" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    ...
                    <option value="{{ $kriteria->id_kriteria }}" {{ old('id_kriteria') == $kriteria->id_kriteria ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_sub" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('nama_sub') }}" required>
            </div>

            <div class="mb-6">
                <label class="block text-gray-700 text-sm font-bold mb-2">Nilai</label>
                <input type="number" step="0.01" name="nilai" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('nilai') }}" required>
            </div>
